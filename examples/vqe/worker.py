...
tick. The local backend defers each result by ``VQE_LOCAL_LATENCY`` (vqe.py),
so this loop -- ticks, mid-job stop, cancel -- actually executes on the
credential-free default path, not only against a real queue.
...
def collect(jobs, handle, w, step, poll_seconds):
...
def main():
    ...
    with runstate.Worker(runstate.attach()) as w:
        ...
        for step in w.steps(start=start, total=cfg["budget"]):
            ...
            counts = collect(jobs, handle, w, step, cfg["poll_seconds"])
            if counts is None:
