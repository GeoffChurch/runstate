...
**v0.3 (in progress, on `master`):** the convention bodies are typed frozen dataclasses in `runstate/vocabulary/`; `value` events carry an absolute wall-clock `value.t`; a `run_id()` *recipe* documents reuse-by-content-hash (`docs/specs/run-id-recipe.md`); the **run-episodes** scoped primitive landed — a `run_id` hosts multiple resumable episodes (relaunch-to-extend), with the single-spawn guard implemented as a worker self-claim (`docs/specs/run-episodes.md`); and the **memoizer** that consumes it shipped — `history()` replays a schedule over the logged `value` points (passive, channel-only), and `ensure(producer, name, up_to=N)` serves the logged prefix on a hit or relaunches-to-extend and waits on a miss, with the `launch_producer` seam + the `relaunch_if_needed` helper (`docs/specs/memoizer.md`). Next up: the deferred service/lifeline memoizer + a named `Producer` Protocol, and the relational-layer **Store** — see `docs/backlog/index.md` "Start here". Full v0.3 trail: `docs/design-v0.3-exploration.md`.
