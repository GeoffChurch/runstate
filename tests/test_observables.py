...
def _episode(ch, *, launch, pid, at):
    ch.send({"handle": f"local://h/{pid}", "t": at}, topic="launcher.launched", request_id=launch)
...
def test_a_claim_losers_clean_exit_does_not_complete_the_run(open_channel):
    ...
    ch.send({"handle": "local://h/2", "t": 0.0}, topic="launcher.launched", request_id="loser")
    ch.send({"reason": "exited", "exit_code": 0, "signal": None, "t": 0.0},
...
def test_a_hand_run_workers_episode_has_no_launcher_verdict(open_channel):
    ...
    ch = open_channel()
    ch.send({"handle": "local://h/9", "t": 0.0}, topic="launcher.launched", request_id="L1")
...
def test_peek_terminal_typed_error_on_extra_key_stopped(open_channel):
    ...
    seq = open_channel().send(
        {"completed": True, "error": None, "final_step": None, "t": 0.0, "oops": 1},
        topic="lifecycle.stopped",
    )
...
def test_peek_terminal_typed_error_on_a_death_that_names_no_launch(open_channel):
    # launcher-v0.4: a death record with no request_id is unattributable -- it
...
# ----- last_activity: the freshness clock a cold observer reads (observer-clock §6) -----


def test_last_activity_none_on_empty(open_channel):
    assert last_activity(open_channel()) is None


def test_last_activity_picks_the_newest_of_the_three_dated_records(open_channel):
    # heartbeat / stopped / terminated each carry a t; last_activity is their max
    # -- the newest by t, NOT the latest by seq (a fast clock can't pin it forever
    # because latest() only reads the current newest of each topic).
    ch = open_channel()
    ch.send({"step": 0, "consumed_seq": 0, "t": 10.0}, topic="lifecycle.heartbeat")
    assert last_activity(open_channel()) == 10.0
    ch.send({"reason": "exited", "exit_code": 0, "signal": None, "t": 25.0},
            topic="launcher.terminated", request_id="L1")
    assert last_activity(open_channel()) == 25.0            # terminated is newer
    ch.send({"completed": False, "error": None, "final_step": 1, "t": 18.0},
            topic="lifecycle.stopped")
    assert last_activity(open_channel()) == 25.0            # max by t, not latest by seq


def test_last_activity_ignores_junk_t(open_channel):
    # a record whose t is absent or wrong-typed contributes nothing (the
    # measurement rule) -- an unmigrated/foreign log never poisons freshness.
    ch = open_channel()
    ch.send({"step": 0, "consumed_seq": 0}, topic="lifecycle.heartbeat")            # no t
    assert last_activity(open_channel()) is None
    ch.send({"step": 1, "consumed_seq": 0, "t": "soon"}, topic="lifecycle.heartbeat")  # wrong-typed
    assert last_activity(open_channel()) is None
    ch.send({"step": 2, "consumed_seq": 0, "t": 7.0}, topic="lifecycle.heartbeat")     # conforming
    assert last_activity(open_channel()) == 7.0
...
def test_last_activity_never_reads_value_records(open_channel):
    ...
    # freshness reads only the beacon/terminal records (observer-clock §3, §6).
    ch = open_channel()
