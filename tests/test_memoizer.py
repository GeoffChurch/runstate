...
def test_history_time_schedule_is_run_relative_to_the_run_epoch(open_run):
    ...
    for step in range(6):
        ch.send(
            {"value": float(step), "step": step, "t": 1000.0 + step},
            topic="value",
            name="loss",
        )  #                    absolute t
...
def test_history_skips_nonconforming_value_records(open_run):
    ...
    ch.send(
        {"value": 1.0, "step": 1}, topic="value", name="loss"
    )  #               missing t
    ch.send(
        {"value": 2.0, "step": "two", "t": 0.0}, topic="value", name="loss"
    )  # wrong-typed step
    ch.send(
        {"value": 3.0, "step": True, "t": 0.0}, topic="value", name="loss"
    )  #  bool is not a step
    ch.send(
...
def test_launch_producer_extend_injects_target_and_runs():
    ...
    assert (
        producer.channel is not None
    )  #                    .channel opens without error
    ...
    assert (
        seen["up_to"] == 3
    )  #                               target injected into worker kwargs
...
def test_ensure_surfaces_a_failure_outcome(tmp_path):
    ...
    assert (
        ei.value.result.outcome in runstate.Outcome.failures()
    )  #  the verdict, at raise time
...
def test_ensure_raises_when_run_makes_no_progress(tmp_path):
    ...
    assert ei.value.progress == 1 and ei.value.until == {
        "step": 5
    }  #  the diagnostics, as data
...
def test_ensure_completed_short_of_up_to_returns_without_redriving():
    ...
    ch = MemoryChannel()
    K = 3
    _seed_episode(ch, heartbeat_step=K, completed=True, value_steps=list(range(K + 1)))
...
def test_ensure_preempted_redrives_then_stops_on_completion():
    ...
    _seed_episode(ch, heartbeat_step=K, completed=False, value_steps=list(range(K + 1)))
...
def test_ensure_killed_resumes_on_caller_re_call_take_the_latest():
    ...
    def episodes(channel, target):
        ...
        if (
            ...
            channel.send(
                {"reason": "killed", "exit_code": None, "signal": 9, "t": 0.0},
...
def test_launch_producer_rejects_non_step_condition():
    ...
    for bad in (
        ...
        with pytest.raises(
            ValueError, match="translates only"
        ):  # matches the real message
            producer.extend(bad)
...
def test_ensure_time_milestone_satisfies_via_poll_clock_even_when_value_sparse():
    ...
    assert [b["step"] for b in series] == [
        0
    ]  #  crossed the budget on the clock, not value.t
...
def test_ensure_compound_all_step_met_time_pending_does_not_false_raise():
    ...
    series = ensure(
        p,
        "loss",
        until={"all": [{"step": 3}, {"time_seconds": 5}]},
        clock=_RampClock(),
        poll_interval=0,
    )
    assert [b["step"] for b in series] == [
        ...
        1,
        2,
    ]  #  window [0,3); both bounds met by return
    assert (
        p.calls >= 2
    )  #                              re-drove after step met, on time -- no raise
...
class _TimeChunkProducer:
    ...
    def __init__(self, channel, completed: bool):
        self._c = channel
    ...
    def extend(self, until):
        ...
        self.calls += 1
        self._c.send(
            {"value": float(self._s), "step": self._s, "t": 0.0},
            topic="value",
            name="loss",
        )
        self._c.send(
...
def test_ensure_time_budget_preempted_accumulates_across_chunks():
    ...
    series = ensure(
        p, "loss", until={"time_seconds": 5}, clock=_RampClock(), poll_interval=0
    )
    assert (
        p.calls >= 2
    )  #                                       re-drove across timed chunks
    assert [b["step"] for b in series] == list(
        range(p.calls)
    )  # one continuous accumulated series
...
def test_ensure_time_budget_completed_truncates_after_first_chunk():
    ...
    assert p.calls == 1 and [b["step"] for b in series] == [
        0
    ]  #  completed -> stopped at chunk 1
        ...
        for _step in w.steps(total=1):
            ...
            for k, v in {"pair_metrics": 0.42, "hubness": 7.0}.items():
                if k not in existing:
                    channel.send(
                        asdict(runstate.Value(value=v, step=0, t=0.0)),
                        topic="value",
                        name=k,
                    )
...
def test_store_pin_latecomer_waits_on_live_foreign_episode():
    ...
    class _Gated:  #                                the Recipe-2 gate, latecomer side
        ...
        def extend(self, until):
            assert (
                runstate.live_episode(ch) is not None
            )  #  the pin gates, never launches
    ...
    def winner_delivers(
        _,
    ):  #                      the foreign winner, via the poll hook
        s = delivered["n"]
        if s < 5:
            ch.send(
                {"value": float(s), "step": s, "t": 0.0}, topic="value", name="loss"
...
def test_store_pin_latecomer_recovers_when_foreign_winner_dies_recordless(tmp_path):
    ...
    def hang_guard(_):
        ...
        if (
            calls["n"] == 1
        ):  #                      ...replaced by a claim that died recordless
            ch.send(
                {"handle": f"local://{host}/2147483646", "t": 0.0},
                topic="lifecycle.started",
            )
        time.sleep(
            0.001
        )  #                        real yield: the recovery runs in a thread
        if calls["n"] > 200:
