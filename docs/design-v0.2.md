...
@dataclass class Envelope:
    seq: int                  # substrate-assigned; the per-log monotonic total order (one sequencer per log)
...
- **A per-log total order is the contract**: `seq` is one monotonic sequence across all topics, assigned by the log's single sequencer (SQLite's autoincrement; Postgres's `(run_id, seq)` key; the Memory list). The conventions depend on it — the pairing-by-`seq` rule (§7) compares positions *across* topics in every instance — so a backend offering only per-topic FIFO cannot host them.
