...
- **Matching is case-sensitive** (SQLite uses GLOB, not the ASCII-case-insensitive
  LIKE; Python uses `str.startswith`; Postgres uses a `text_pattern_ops` /
  C-collation prefix range).
...
reading fresh (`watcher._seed_beacon`; observer-clock §5). `last_activity` reads
the freshness clock directly: the **newest `t`** among `latest(heartbeat)` /
`latest(stopped)` / `latest(terminated)` — never a `max` over the whole log (one
fast-clock record would pin a whole-log max into the future forever, and this
feeds the GC's irreversible action — observer-clock §6).
...
### 5.1 The `attach` environment variables

A launcher sets these in the worker's environment; the reference `attach()` reads
them (explicit arguments override — `runstate/__init__.py:attach`). A
second-language launcher sets the **same** variables:
