...
| **4 — read-only open** | **UNSETTLED — a real runstate question.** `open_channel` creates a `<rid>.db` on a missing run, so a stale pointer manufactures a phantom into a content-addressed store. Candidates, least-commitment first: **stat-before-open** (app-side, no runstate change) or **`create=False`** (graded *minor*; the `lazy creation` alternative was refuted — see git). Decide when the resolver first hits a stale rid. |
