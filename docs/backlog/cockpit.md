...
| **4 — read-only open** | **SETTLED — shipped ([`../specs/channel-locators.md`](../specs/channel-locators.md)).** The cockpit's glob resolver first hitting a stale rid was exactly the trigger this ledger said to wait for. Resolution: **split the locator, kill the flag** — `attach_channel` (existing-only) raises `RunNotFound` on a missing/empty/foreign store and **never mutates a foreign valid sqlite db** (the sharp `executescript`-at-open harm, pinned byte-identical), while `create_channel` is the one explicitly-named birth. The resolver opens with `attach_channel` and catches `RunNotFound`; no `create=False` flag, no stat-before-open (which couldn't catch the foreign-db mutation anyway). |
