...
5. **Is every *regular* theory of presheaf type?** Gabriel–Ulmer settles cartesian affirmatively; Beke
   exhibits a coherent theory (fields) that is not. Regular sits between. **Attempted 2026-08-20, not
   resolved, but reduced** — and the framing was probably wrong: the literature speaks of *characterisations
   of* the regular theories classified by a presheaf topos, and a characterisation is only interesting for a
   proper subclass, so this is likely **known with the answer "no"** and what is missing is a citation, not
   a result. Caramello's work on theories of presheaf type is where to look.

   **The reduction, which stands either way.**

   - **Criterion:** `T` is of presheaf type **iff** it is classified by `[C, Set]` with
     `C := f.p. T-mod(Set)` — presheaves on its own finitely presentable models. Necessary condition:
     `Mod(T)` is **finitely accessible**.
   - **Regular theories are exactly injectivity classes.** A regular sequent `φ(x̄) ⊢ ∃ȳ ψ(x̄,ȳ)` says that
     for the finitely presentable `A` (freely generated by `x̄` with `φ`) and `B` (by `x̄,ȳ` with `φ ∧ ψ`),
     every homomorphism `A → M` factors through `A → B`. That is injectivity with respect to a map between
     finitely presentable objects.
   - **And injectivity classes are accessible but not necessarily *finitely* accessible.** That gap is
     exactly where a counterexample lives, and it is why cartesian is clean: Gabriel–Ulmer gives a locally
     finitely presentable model category outright.
   - **Candidate counterexample: divisible abelian groups**, `⊤ ⊢_x ∃y. ny = x` for each `n` — regular, with
     an existential head and no `∨` or `⊥`. Suspicion: its finitely presentable objects are too sparse for
     every model to be a filtered colimit of them, so the category is not finitely accessible.
   - **The one checkable question it reduces to:** *is `ℚ` finitely presentable in the category of divisible
     abelian groups?* Unverified — a map out of `ℚ` needs coherent choices of every `1/n` and no single
     stage of a filtered diagram obviously supplies them, but that is a suspicion, not an argument.
