...
- **[ensure-redrive-recoverable-terminations](ensure-redrive-recoverable-terminations.md)** — **RESOLVED 2026-06-27**, shipped as **G1 + a caller recipe**, not an in-`ensure` feature. The need: re-drive a killed/timed-out run that made progress. The resolution: `ensure` is *unchanged* — it auto-continues `preempted` stops (routine, policy-free) and **fails fast on a death** (exceptional, policy-laden); the caller catches the death, applies its own retry budget, and **re-calls `ensure`** to resume from the checkpoint (read-first → relaunch + resume), with [G1](value-plane-divergence-resolution.md) take-the-latest absorbing the resumed overlap. That keeps the retry budget + per-attempt visibility in the caller (where the budget is) and off the core — dissolving the in-library give-up bound + thrash-observability a `recoverable()`-hook-in-`ensure` would have forced. Two reframes refuted en route (`../dead_ends/ensure-extend-pushorfail.md`, `failure-detector.md`); a recoverability-flip + producer hook was *implemented then reverted* (fail-fast is simpler and stays on the right side of "transports messages, not processes"). The self-diagnosed-fatal discriminator (`error is not None`) lives in the caller's retry predicate. Deferred (YAGNI): a public `extend_once`/`satisfied` + the own-loop recipe for exotic *stop* policies.
- **[value-plane-divergence-resolution](value-plane-divergence-resolution.md)** — the keystone (the redrive item's "G1"): when a resumed run re-emits an overlapping step with a divergent value, `history` refuses reuse *forever* (sticky, append-only). Fix (after a code-grounded red-team refuted an earlier attribution/authoritative-resolution draft): **resolve by take-the-latest and delete the raise** — a read-time fold, no log mutation, matching the fold `value_series` already ships; *removes* code. Sound for every divergence `ensure` can produce (`ensure` never re-drives a `completed` run → the finished-then-rerun counterexample is unreachable). **Also fixes a latent bug today**: the shipped `preempted`-redrive can already sticky-poison reuse on a non-reproducible worker. (The earlier "close the `relaunch_if_needed`/`ensure_served` hole" sub-fix was **dropped** — redundant for the former, wrong for the latter's leased re-wake model.) Unblocks killed-redrive and the cross-host claim gate, both of which need only non-stickiness. Attribution/fork-surface deferred (YAGNI).
...
- **Cross-host liveness for the claim gate** — `live_episode` sits at the probe-only
  ...
  *definitive* cross-host oracle — a connection-oriented backend's lock (a Postgres
  ...
  value-plane robustness ([value-plane-divergence-resolution](value-plane-divergence-resolution.md)) so a residual double-live can't sticky-poison the log;
