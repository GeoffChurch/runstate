...
- **[ensure-redrive-recoverable-terminations](ensure-redrive-recoverable-terminations.md)** — let `ensure` re-drive killed/timed-out runs that made progress, instead of raising; subsumes the consumer's custom resume loop. Surfaced by mycooc Phase-4 dogfood (the `_SyncHandle` terminal-synthesis and `_run_one_chunk` resume loop are the dual of this missing feature). The **push-or-fail `extend` reframe was REFUTED** 2026-06-25 (`../dead_ends/ensure-extend-pushorfail.md`): the preempt/kill asymmetry is *protective* (clean-final-checkpoint ⟺ no-overlap ⟺ safe-redrive), so re-driving `killed` writes the overlap that *sticky-poisons* reuse. Corrected shape: keep `ensure`'s loop; replace the `_FAILURES`→raise with a **canonical recoverability function** (`Outcome` + self-reported `error` + progress — no producer-predicate default), gated per outcome-tier — `preempted` now, `killed` behind **G1 (non-sticky `history`)**, `presumed_dead` behind the claim oracle, `errored` never — bounded by `max_attempts`. **G1 is the same value-plane keystone the cross-host claim gate needs** (below); two reframes have died on it. Not bit-exact-testable; needs a mock-producer (+ divergence) approach.
