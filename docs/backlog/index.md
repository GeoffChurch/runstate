...
- **[ensure-redrive-recoverable-terminations](ensure-redrive-recoverable-terminations.md)** — let `ensure` re-drive killed/timed-out runs that made progress instead of raising; removes a resume loop consumers hand-roll (mycooc's `_run_one_chunk` RESUME branch + `_SyncHandle` terminal-synth — ~18 lines, *not* the process-management bulk). Two reframes refuted en route (`../dead_ends/ensure-extend-pushorfail.md`, `failure-detector.md`). **Converged + red-teamed** (2026-06-26): safe-to-redrive is a *checkpoint/reproducibility* property, not the outcome — so the shipped `preempted`-redrive is *already* a latent poison and **G1 fixes it**. Recoverability: the library decides only `error is not None`⟹fatal and `completed`⟹done; the non-self-diagnosed deaths (`killed`, recordless-exit) are a **producer** judgment via an optional `recoverable(result)` hook, **conservative default = today's raise** (so the seam is *additive*). Staged: **G1** (take-the-latest; removes code, fixes the preempt poison) → the additive recoverability seam → post-G1 `KILLED`-redrive + a `max_attempts` bound + thrash-observability. Deferred (YAGNI): a public `extend_once`/`satisfied`, the primitive-basis formalization, the own-loop recipe (non-functional as sketched), the error-string override.
