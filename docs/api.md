...
freshness clock a third-party observer reads. The newest `t` among the latest of
**all five** dated topics — `started` / `heartbeat` / `stopped` (lifecycle) and
`launched` / `terminated` (launcher) — five O(1) reads, never a `max` over the
whole log. `started`/`launched` are load-bearing, not padding: a run that
attached or was spawned but has not beaconed yet must still have an age, or a
GC's "skip homes younger than T" can't protect a genuinely-recent home.
(`value` is excluded — its `t` is present-nullable, the documented blind spot
for a convention-opt-out worker.) None if no dated record exists yet.
