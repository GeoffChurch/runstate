...
> **Not on PyPI yet.** The release machinery is prepared but the first release
> is owner-gated (`backlog/release-and-stability-contract.md`); until it lands,
> install from a clone: `pip install -e .`
...
**`preempted`** — a clean, *resumable* stop. A resumable or chunked worker (§4)
...
`Condition` — a threshold `{"step": N}` / `{"time_seconds": S}`, or
`{"any": [...]}` (first to cross) / `{"all": [...]}` (last to cross); the
`count` coordinate is legal only under `until` (below):
...
## 5. The episode rule (one run_id, many episodes)
...
## 6. Five pitfalls (with one-line fixes)
...
5. **Checkpoint what you *did*, not what you were *asked* to do.** Writing the
   ...
   loop, after each step's work is durable (§4).
