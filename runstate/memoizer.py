...
def _epoch(channel: Channel) -> float | None:
    ...
    run has none (never started, or a junk-typed ``t``: junk earns no epoch, the
    measurement rule). ``t`` is required non-null on a conforming started record
    (lifecycle-v0.4), so the legit-null branch is gone; only a foreign/unmigrated
    log's absent-or-wrong-typed ``t`` still yields None (returning None there is
    existing tested behavior). The ONE epoch reader ``history`` and ``_elapsed``
    share; their null-epoch responses differ (raise vs. inert) but the epoch
    itself cannot."""
...
def history(channel: Channel, name: str, schedule: Condition) -> list[Body]:
    ...
    if epoch is None:
        if references_time(schedule):
            raise ValueError("time-referencing replay requires a run epoch (started.t)")
        epoch = 0.0
